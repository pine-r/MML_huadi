...
<html>
  <head>
	...
	<script type="text/javascript" src="js/jquery-1.11.min.js"></script>
	<% String id1=request.getParameter("goodsId"); 
	...
	$(document).ready(function(){
		...
		var name=$(".sku_name");
		var infoId=$(".info_wrap");
		var num=$("#num").val();
		$.ajax({
			type:"post",
			url:"http://localhost:8080/hwadee/goods/detail.shtml",
			data:{
			"goodsId":id,
			},
			success:function(data){
				if(data.success){
					img.attr("src",data.data.goodsImages);
					price.text("￥"+data.data.goodsPrice);
					name.html(data.data.goodsName+"<br><br>");
					infoId.attr("id",data.data.goodsId);
				}
			},
		...
		$("#addCart").click(function(){
			var num=$("#num").val();//就是要在点击后获取输入值，不然网页一加载的时候是不行的，没值
			$.ajax({
				type:"post",
				url:"http://localhost:8080/hwadee/basket/add.shtml",
				data:{
				"goodsId":id,
				"count":num,
				},
  ...
  <body>
    <div class="container">
		...
		<div class="info_wrap" >
			<div class="info_pic">
				<img src="images/小米红米6.jpg" id="good_img">
			</div>
			<div class="basic_info">
				...
				<div class="price">
					米米乐价：<span id="goods_price"></span><br><br>
					会员价：<br><br>
				</div>
				...
				<div class="select_num">
					<div class="input-group input-group-sm" style="width: 150px;height: 30px;float: left;">
						...
						<input type="text" class="number form-control input-sm"  id="num"/>
				
					</div>
					<div class="info_button">
						<button name="加入购物车" id="addCart">加入购物车</button>
